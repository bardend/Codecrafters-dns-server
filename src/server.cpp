...
int main() {
   ...
   while (true) {
       // Receive data
       bytesRead = recvfrom(udpSocket, buffer.data(), buffer.size(), 0, reinterpret_cast<struct sockaddr*>(&clientAddress), &clientAddrLen);
       if (bytesRead == -1) {
           perror("Error receiving data");
           break;
       }

       buffer.resize(bytesRead);  // Adjust buffer size to actual bytes read
       buffer.push_back(0x00);    // Null-terminate if needed
